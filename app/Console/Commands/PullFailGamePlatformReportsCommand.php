...
class PullFailGamePlatformReportsCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'southeast:pull-fail-game-platform-reports';
    ...
    public function handle()
    {
        foreach (GamePlatform::getAll()->where('status', true) as $platform) {
            # 获取时刻表
            foreach ($platform->getNoProcessSchedules($platform->limit, GamePlatformPullReportSchedule::STATUS_FAIL) as $schedule) {
