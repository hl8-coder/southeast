...
class SyncThBankNumberCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'southeast:sync-th-bank-number';
    ...
    public function handle()
    {
        ...
        try {
            $this->info('总共要导 ' . $this->ThUserBankAccounts->count() . ' 条');
            DB::transaction(function() {
                $bankCodes = $this->ThUserBankAccounts->pluck('bank_code')->unique();

                $banks = Bank::query()->whereIn('code', $bankCodes)->get();

                foreach ($this->ThUserBankAccounts as $k => $v) {
                    # 找出对应user
                    $logs = SyncThLog::query()->where('old_id', $v['mem_id'])->where('status', true)->get();
                    $bank = $banks->where('code', $v['bank_code'])->first();
                    foreach ($logs as $log) {
                        UserBankAccount::query()->where('user_id', $log->new_id)
                            ->where('bank_id', $bank->id)
                            ->update([
                                'account_no' => $v['acct_num'],
                            ]);
